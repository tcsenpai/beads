...
func FlushWorkingSet(host string, port int) error {
	...
	dsn := fmt.Sprintf("root@tcp(%s:%d)/?parseTime=true&allowNativePasswords=true&allowCleartextPasswords=true", host, port)
...
func waitForReady(host string, port int, timeout time.Duration) error {
	...
	for time.Now().Before(deadline) {
		...
		dsn := fmt.Sprintf("root@tcp(%s)/?parseTime=true&allowNativePasswords=true&allowCleartextPasswords=true&timeout=2s", addr)
		db, err := sql.Open("mysql", dsn)
		if err == nil {
